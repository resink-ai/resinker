...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Resinker - A YAML-based configuration system for mocking event streams")
    
    subparsers = parser.add_subparsers(dest="command", help="Command to run")
    
    # Run command
    run_parser = subparsers.add_parser("run", help="Run a simulation")
    run_parser.add_argument("-c", "--config", required=True, help="Path to the YAML configuration file")
    run_parser.add_argument("-v", "--verbose", action="store_true", help="Enable verbose logging")
    
    # Validate command
    validate_parser = subparsers.add_parser("validate", help="Validate a configuration file")
    validate_parser.add_argument("-c", "--config", required=True, help="Path to the YAML configuration file")
    
    # Info command
    info_parser = subparsers.add_parser("info", help="Display information about a configuration file")
    info_parser.add_argument("-c", "--config", required=True, help="Path to the YAML configuration file")
...
def info_command(args: argparse.Namespace):
    ...
    try:
        ...
        print(f"  Total Events: {config.simulation_settings.total_events or 'Not specified'}")
        print(f"  Random Seed: {config.simulation_settings.random_seed or 'Not specified'}")
        print(f"  Start Time: {config.simulation_settings.time_progression.start_time}")
        print(f"  Time Multiplier: {config.simulation_settings.time_progression.time_multiplier}")
        
        # Initial entity counts
        if config.simulation_settings.initial_entity_counts:
            print("\nInitial Entity Counts:")
            for entity_type, count in config.simulation_settings.initial_entity_counts.items():
        ...
        if config.outputs:
            ...
            for i, output in enumerate(config.outputs):
                ...
                print(f"  {i+1}. {output_type} ({'enabled' if enabled else 'disabled'})")
