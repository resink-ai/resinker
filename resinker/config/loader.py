...
class SimulationSettings(BaseModel):
    ...
    time_progression: TimeProgressionSettings = Field(default_factory=TimeProgressionSettings)
    ...
    def validate_duration(cls, v):
        ...
        if not match:
            raise ValueError(f"Invalid duration format. Use '<number><unit>' where unit is s, m, or h. Example: '30m', '1h', '10s'")
...
class ScenarioDefinition(BaseModel):
    ...
    requires_initial_entities: List[ScenarioEntityRequirement] = Field(default_factory=list)
