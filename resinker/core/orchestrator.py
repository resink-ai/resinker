...
class ScheduledEvent:
    ...
    def __init__(self, event_type: str, scheduled_time: datetime, context: Dict[str, Any] = None):
        self.event_type = event_type
...
class Orchestrator:
    ...
    def __init__(self, config: ResinkerConfig):
        ...
        self.time_multiplier = config.simulation_settings.time_progression.time_multiplier
    ...
    def initialize(self):
        ...
        for entity_type, count in self.config.simulation_settings.initial_entity_counts.items():
    ...
    def run(self):
        ...
        while True:
            ...
            elapsed_sim_time = (self.simulation_time - self.simulation_start_time).total_seconds()
            ...
            if not self.event_queue:
                ...
                if not self.event_queue:
                    logger.warning("No more events can be scheduled. Ending simulation.")
                    break
            ...
            if event:
                ...
                if event_count % 100 == 0:
                    logger.info(f"Generated {event_count} events. Current simulation time: {self.simulation_time}")
        ...
        logger.info(f"Simulation complete. Generated {event_count} events in {real_elapsed:.2f} seconds.")
    ...
    def _can_generate_event(self, event_type: str) -> bool:
        ...
        if "consumes_entities" in event_def:
            for consumption in event_def["consumes_entities"]:
                ...
                entity_count = self.state_manager.count_entities(entity_type, filter_list)
    ...
    def _generate_event(self, scheduled_event: ScheduledEvent) -> Optional[Event]:
        ...
        if "updates_entity_state" in event_def:
            for update in event_def["updates_entity_state"]:
                ...
                if entity is None:
                    logger.warning(f"Entity with alias {entity_alias} not found for state update")
                    continue
