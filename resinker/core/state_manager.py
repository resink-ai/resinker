...
class Entity:
    ...
    def increment_state(self, key: str, value: Any):
        ...
        if isinstance(current, (int, float)) and isinstance(value, (int, float)):
            ...
            raise ValueError(f"Cannot increment non-numeric state {key} with value {value}")
...
class StateManager:
    ...
    def register_entity_type(self, entity_type: str, primary_key: str):
        ...
        if entity_type not in self.entities:
            ...
            logger.info(f"Registered entity type: {entity_type} with primary key: {primary_key}")
    
    def create_entity(self, entity_type: str, data: Dict[str, Any], primary_key: str) -> Entity:
    ...
    def update_entity(self, entity_type: str, entity_id: str, data: Dict[str, Any]) -> Optional[Entity]:
        ...
        if entity_type in self.entities and entity_id in self.entities[entity_type]:
            ...
            logger.warning(f"Entity {entity_type} with ID {entity_id} not found for update")
            return None
    ...
    def update_entity_state(
        ...
        if entity_type in self.entities and entity_id in self.entities[entity_type]:
            ...
            logger.debug(f"Updated state for entity {entity_type} with ID {entity_id}")
            return entity
        else:
    ...
    def find_entities(self, entity_type: str, filters: List[Dict[str, Any]], limit: Optional[int] = None) -> List[Entity]:
    ...
    def count_entities(self, entity_type: str, filters: Optional[List[Dict[str, Any]]] = None) -> int:
    ...
    def delete_entity(self, entity_type: str, entity_id: str) -> bool:
        ...
        if entity_type in self.entities and entity_id in self.entities[entity_type]:
            ...
            return True
        logger.warning(f"Entity {entity_type} with ID {entity_id} not found for deletion")
        return False 