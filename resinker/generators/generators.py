...
class ConditionalChoiceGenerator(Generator):
    ...
    def generate(self, schema: Dict[str, Any], context: Dict[str, Any] = None) -> Any:
        ...
        if condition_value is None:
            ...
            raise ValueError(f"Condition field '{condition_field}' not found in context and no default case provided")
        ...
        for case in cases:
            ...
            if "condition_value_greater_than" in case and condition_value > case["condition_value_greater_than"]:
                return self._choose_from_case(case)
            
            if "condition_value_less_than" in case and condition_value < case["condition_value_less_than"]:
                return self._choose_from_case(case)
            
            if "condition_value_in" in case and condition_value in case["condition_value_in"]:
                return self._choose_from_case(case)
        ...
        raise ValueError(f"No matching case found for condition '{condition_field}' with value '{condition_value}'")
...
class StaticHashedGenerator(Generator):
    ...
    def generate(self, schema: Dict[str, Any], context: Dict[str, Any] = None) -> str:
        ...
        if raw_value_source:
            ...
            raw_value = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(12))
...
class FromEntityGenerator(Generator):
    ...
    def generate(self, schema: Dict[str, Any], context: Dict[str, Any] = None) -> Any:
        ...
        if not from_entity or not field:
            raise ValueError("FromEntityGenerator requires from_entity and field parameters")
        ...
        if entity is None:
            ...
            for alias, entities in consumed_entities.items():
                if alias == from_entity or (isinstance(entities, list) and len(entities) > 0 and entities[0].entity_type == from_entity):
...
class SchemaGenerator(Generator):
    ...
    def __init__(self, faker: Faker, state_manager: StateManager = None, schema_registry: Dict[str, Any] = None):
    ...
    def _generate_object(self, schema: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_array(self, schema: Dict[str, Any], context: Dict[str, Any]) -> List[Any]:
    ...
    def _generate_number(self, schema: Dict[str, Any], context: Dict[str, Any]) -> float:
    ...
    def _generate_boolean(self, schema: Dict[str, Any], context: Dict[str, Any]) -> bool:
...
def generator_factory(generator_type: str, faker: Faker, state_manager: StateManager = None) -> Generator:
